...
try {
    ...
    SET bio = ?, hobby1 = ?, hobby2 = ?, hobby3 = ?, hobby4 = ?, favorite1 = ?, favorite2 = ?, favorite3 = ?, favorite4 = ?, dateModified = ?
    WHERE customer_ID = ?');
    $updateAccount->execute([$bio, $hob1, $hob2, $hob3, $hob4, $fav1, $fav2, $fav3, $fav4, $dateModified, $customerID]);
    ...
    $insertActivity->execute(["Updated background information", $customerID]);

    $conn->commit();
    echo '<script>alert("Background Information Updated Successfully!");';
    echo 'window.location.href = "http://localhost/DevBugs/account_main.php";</script>';
} catch (\Throwable $th) {
    echo $th;
    $conn->rollBack();
}
